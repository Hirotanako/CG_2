...
void ParticleUpdateCS(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    if (p.age >= p.lifetime)
    {
        ...
        const float safeEmitterY = max(
            EmitterAndDeltaTime.y, TimeAndSize.z + TimeAndSize.y);
        p.position = float3(
    ...
    {
        // Position integration happens only in this compute shader.  Initial
        // Y speed is at least 0.85 and acceleration is positive, so velocity.y
        // cannot turn downward during a particle's lifetime.
        p.velocity.y += TimeAndSize.w * dt;
        p.position += p.velocity * dt;
    }

    // This is deliberately the final operation before Append: neither an
    // integration step nor a respawn can leave a billboard below the floor.
    const float minimumParticleY = TimeAndSize.z + TimeAndSize.y;
    if (p.position.y < minimumParticleY)
    {
        p.position.y = minimumParticleY;
